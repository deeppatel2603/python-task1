...
if ("can" in x)or ("you" in x) and ("youtube" in x) or ("play song" in x):
...
if ("run"in x) or ("execute"in x) and ("notepad"in x) or ("editor"in x):
    os.system("notepad")
...
if ("read"in x) or ("scan" in x) and ("file"in x) or ("textfile" in x):
    ...
    f=open(y,'r')
    print(f.read())
if ("note"in x) or ("create" in x) and ("file"in x) or ("textfile" in x): 
